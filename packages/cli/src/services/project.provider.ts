...
export class ProjectProvider {
    ...
    constructor(injector: InjectorProvider) {
        ...
        this.shell = injector.getProvider(ShellCommandsProvider)
    }
    ...
    async refreshAppData(cwd?: string) {
        ...
        try {
            await this.shell.executeCommand("mvn clean spring-boot:run -q -D\"spring-boot.run.arguments\"=\"--socle.devtools.enabled=true --socle.devtools.close-on-startup=true --server.port=55555 --spring.main.log-startup-info=false --logging.level.ROOT=off\"", paths?.server_root_path);
