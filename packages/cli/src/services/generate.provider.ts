...
import {PostGenerateControllerBody, PostGenerateEntityBody, PostGeneratePageBody, PostGenerateReferentielBody, PostGenerateStoreBody, PostGenerateTimerBody, ProjectPaths} from '@kraken/types'
...
export class GenerateProvider {
    ...
    async generateEntity(body: PostGenerateEntityBody, paths?: ProjectPaths | null) {
        ...
        const name = stringcase.pascalcase(data.name)
    ...
    async generateStore(body: PostGenerateStoreBody, paths?: ProjectPaths | null) {
        const {cwd, ...data} = body
