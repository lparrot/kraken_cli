...
export class ShellCommandsProvider {
  ...
  async runApplication(cwd: string, check = false) {
    ...
    if (!check || (shell.which("mvn") != null && shell.which("npm") != null)) {
      ...
      netstat({sync: true, filter: {local: {port: appdata?.port}}}, item => {
        processes = item
      });
      ...
      const mvn = child_process.spawn('mvn clean spring-boot:run -q -D\"spring-boot.run.arguments\"=\"--spring.output.ansi.enabled=always --spring.profiles.active=dev\"', {cwd: paths?.server_root_path, shell: true})
